...
@IonicPage()
@Component({
  selector: 'page-pick-location',
  templateUrl: 'pick-location.html',
})
export class PickLocationPage {
  ...
  constructor(
    ...
    this.requestId = this.navParams.get('data')
  ...
  setLocation() {
    ...
    this.locationArray = [{ Lat: this.lat, Lon: this.lng }];
    let requestData = {
    ...
    this.http.post('http://constructionlkapi.azurewebsites.net/ItemService/AddItemLocations', requestData, options)
      .subscribe(data => {
